...
- **Self-copy.** Each chat message is also wrapped to the sender's own key and published
  ...
  side of a conversation. Control frames are not copied.
...
- **Sync.** The subscription rewinds three days past the last cursor, because wrap
  timestamps are fuzzed up to two days backwards, and re-evaluates that on every
  reconnect. Duplicates are free — dedup is by event id, and a duplicate is skipped before
  it is even parsed. A catch-up read runs on start and after a read relay drops.
