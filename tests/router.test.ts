...
describe('hash routing', () => {
  ...
  it('takes a group only by a 16-byte hex id, never by a public key', () => {
    expect(parseHash('#/g/' + HEX).name).toBe('chats')
    expect(parseHash('#/g/' + 'B'.repeat(32)).name).toBe('chats')
    expect(parseHash('#/g/').name).toBe('chats')
    expect(parseHash('#/g/' + GROUP + '/elsewhere')).toEqual({ name: 'group', id: GROUP })
  })
