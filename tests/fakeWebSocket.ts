...
export interface RelayBehaviour {
  ...
  natIdleMs: number
  /**
   * Speaks NIP-77. Off, a NEG-OPEN gets the NOTICE that relays without it
   * send for any command they do not know.
   */
  negentropy: boolean
}
...
interface Connection {
  ...
  negotiations: Map<string, NegentropyResponder>
...
export class FakeRelayServer {
  ...
  /** Every event sent in answer to a REQ, stored or live, for asserting traffic. */
  eventsSent = 0
  /** Every NEG-OPEN received. */
  negOpens = 0
  ...
  receive(connection: Connection, data: string): void {
    ...
    switch (frame[0]) {
      case 'REQ': {
        ...
        setTimeout(() => {
          ...
          for (const event of this.stored) {
            if (filters.some((filter) => filter.limit !== 0 && matchesWithIds(filter, event))) {
              this.eventsSent += 1
      ...
      case 'NEG-OPEN': {
        const id = frame[1] as string
        this.negOpens += 1
        if (!this.behaviour.negentropy) {
          this.#send(connection, ['NOTICE', 'ERROR: bad msg: unknown cmd'])
          return
        }
        const { limit: _limit, ...filter } = frame[2] as Filter
        const responder = new NegentropyResponder(
          this.stored.filter((event) => matchesWithIds(filter, event)),
        )
        connection.negotiations.set(id, responder)
        setTimeout(() => this.#send(connection, ['NEG-MSG', id, responder.reply(frame[3] as string)]), 0)
        return
      }
      case 'NEG-MSG': {
        const id = frame[1] as string
        const responder = connection.negotiations.get(id)
        if (!responder) {
          this.#send(connection, ['NEG-ERR', id, 'closed: no such negotiation'])
          return
        }
        setTimeout(() => this.#send(connection, ['NEG-MSG', id, responder.reply(frame[2] as string)]), 0)
        return
      }
      case 'NEG-CLOSE':
        connection.negotiations.delete(frame[1] as string)
        return
      case 'EVENT': {
        this.eventFrames += 1
  ...
  #store(event: NostrEvent): void {
    ...
    for (const connection of this.connections) {
      for (const [id, filters] of connection.subs) {
        // `limit` bounds only the stored replay; live events always flow.
...
/** `matches` from the fake relay, which checks `ids` too — probes and fetches by id use it. */
