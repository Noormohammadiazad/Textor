...
describe('vault lifecycle', () => {
  ...
  /** Rewrite the stored passphrase slot's KDF parameters, as an attacker with the disk could. */
  async function tamperParams(params: unknown): Promise<void> {
    const row = await db.meta.get('keyslots')
    const [slot] = row?.v as Record<string, unknown>[]
    await db.meta.put({ k: 'keyslots', v: [{ ...slot, params }] })
  }
  ...
  it('rejects a tampered KDF parameter block', async () => {
    ...
    vault.lock()
    await tamperParams({ algo: 'scrypt', N: 3, r: 8, p: 1 })
  ...
  it('rejects an absurd memory cost that would exhaust the device', async () => {
    ...
    await vault.create('params', { params: TEST_KDF })
    vault.lock()
    await tamperParams({ algo: 'scrypt', N: 2 ** 30, r: 16, p: 16 })
