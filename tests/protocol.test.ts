...
describe('attachment chunk frames', () => {
  ...
  it('names the copy a chunk or request is for, and refuses a malformed one', () => {
    // ADR-052: one payload can be held under several keys. Frames from before
    // it name none, and still parse.
    const copy = 'c'.repeat(32)
    const request = { v: PROTOCOL_VERSION, t: 'blobreq' as const, id: 'b'.repeat(64), copy, need: [0] }
    expect(parseControlFrame(encodeControlFrame({ ...chunk, copy }))).toEqual({ ...chunk, copy })
    expect(parseControlFrame(encodeControlFrame(request))).toEqual(request)
    for (const bad of ['C'.repeat(32), 'c'.repeat(64), 7]) {
      expect(parseControlFrame(JSON.stringify({ ...chunk, copy: bad }))).toBeNull()
      expect(parseControlFrame(JSON.stringify({ ...request, copy: bad }))).toBeNull()
    }
  })
})
...
describe('NIP-10 threading', () => {
  const root = 'a'.repeat(64)
  const parent = 'b'.repeat(64)

  it('writes a root marker, and only one tag when the parent is the root', () => {
    expect(threadTags(root, root)).toEqual([['e', root, '', 'root']])
    expect(threadTags(root, parent)).toEqual([
      ['e', root, '', 'root'],
      ['e', parent, '', 'reply'],
    ])
  })

  it('threads a forward-secret group message the same way, with no p tags', () => {
    const ts = 1_700_000_000_123
    expect(groupChatTags({ ts })).toEqual([timestampTag(ts)])
    // A reply whose root is not known yet is a reply to a root.
    expect(groupChatTags({ ts, replyTo: parent })).toEqual([...threadTags(parent, parent), timestampTag(ts)])
    expect(groupChatTags({ ts, replyTo: parent, rootId: root })).toEqual([
      ...threadTags(root, parent),
      timestampTag(ts),
    ])
  })

  it('reads the marked form', () => {
    expect(threadFromTags(threadTags(root, parent))).toEqual({ root, replyTo: parent })
    expect(threadFromTags(threadTags(root, root))).toEqual({ root, replyTo: root })
  })

  it('reads the deprecated positional form other clients still send', () => {
    // One tag: a reply to the root itself.
    expect(threadFromTags([['e', root]])).toEqual({ root, replyTo: root })
    // Several: first is the thread, last is the parent.
    expect(
      threadFromTags([
        ['e', root],
        ['e', parent],
      ]),
    ).toEqual({ root, replyTo: parent })
  })

  it('does not treat a quote as a parent', () => {
    // A mention grafted into the thread would pull unrelated messages in.
    expect(threadFromTags([['e', root, '', 'mention']])).toEqual({ root: null, replyTo: null })
  })

  it('ignores tags that are not event ids', () => {
    expect(
      threadFromTags([
        ['p', root],
        ['e', 'nope'],
      ]),
    ).toEqual({ root: null, replyTo: null })
  })
})

describe('reaction bodies', () => {
  it('accepts emoji and NIP-25 plus or minus', () => {
    for (const body of ['👍', '🎉', '😀', '+', '-', '❤️', '👍🏽']) {
      expect(isReactionBody(body)).toBe(true)
    }
  })

  it('refuses anything that could smuggle text into a reaction bar', () => {
    for (const body of ['nice work', 'a', '', '   ', '👍 nice', ':shortcode:', '1', 'x'.repeat(40)]) {
      expect(isReactionBody(body)).toBe(false)
    }
  })
})
