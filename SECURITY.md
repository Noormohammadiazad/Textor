...
- Bypassing vault encryption, the lock, or the check any keyslot makes (passphrase,
  recovery phrase, PIN or pattern and its lockout, the biometric gate) through Textor's own
  interface. Opening a biometric or instant slot, or resetting the PIN count, from a copy
  of the browser's data or from code already running in the page is documented, not a
  bypass: see §3.11 of the threat model
...
- A user choosing a weak passphrase, or choosing to open instantly — see §3.11
