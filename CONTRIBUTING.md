...
knows what breaks if it is deleted. Forward-secret groups (`src/core/mls/`), inbox sync,
negentropy, attachment transfer (`blobTransfer.ts`, `blobCrypto.ts`), and every way the
vault opens (`vault.ts`, `keyslots.ts`, `biometricGate.ts`, `biometricEnrol.ts`,
`exportImport.ts`) are held to 100% of lines and branches by
`vitest.config.ts`: a path through them that no test runs fails `verify`.
