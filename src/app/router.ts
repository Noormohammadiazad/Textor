 ...
 *
 * Hand-rolled rather than react-router because the surface is a handful of routes;
...
export type Route =
  ...
  /**
   * A group, by its conversation id. That id is blinded with this vault's key,
   * so the address bar and browser history name no one — unlike `chat`, whose
   * address is the peer's public key.
   */
  | { name: 'group'; id: string }
  | { name: 'group-info'; id: string }
  | { name: 'new-group' }
  | { name: 'contacts' }
...
export function parseHash(hash: string): Route {
  ...
  switch (head) {
    ...
    case 'g': {
      const id = rest[0] ?? ''
      if (!GROUP_ID.test(id)) return { name: 'chats' }
      return rest[1] === 'info' ? { name: 'group-info', id } : { name: 'group', id }
    }
    case 'new-group':
