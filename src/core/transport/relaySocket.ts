...
/**
 * A NIP-77 negentropy exchange in progress on this socket.
 *
 * The socket only carries the frames; the reconciliation itself lives in the
 * pool. A session does not survive a reconnect: the relay forgets it, so the
 * socket reports an error and the owner starts again if it still wants to.
 */
export interface NegentropySession {
  filter: Filter
  /** The NEG-OPEN payload, hex. */
  initial: string
  onMessage: (messageHex: string) => void
  /** `refused` is true when the relay says it does not do NIP-77 at all. */
  onError: (reason: string, refused: boolean) => void
}

/** How far one subscription has got on the current connection. */
export interface SubscriptionProgress {
  /** When its REQ was last sent, epoch ms; 0 if never. */
  reqAt: number
  /** When the relay finished sending stored events for that REQ; 0 until then. */
  eoseAt: number
}
...
/** How relays that do not implement NIP-77 answer a NEG-OPEN. */
const UNKNOWN_COMMAND =
  /unknown|unsupported|not supported|unrecogni[sz]ed|could not parse|bad msg|invalid (?:message|command)|neg-open/i
...
export class RelaySocket {
  ...
  #progress = new Map<string, SubscriptionProgress>()
  #publishes = new Map<string, PendingPublish>()
  #negotiations = new Map<string, NegentropySession & { answered: boolean }>()
  ...
  /** When anything last arrived from the relay, epoch ms. */
  get lastRxAt(): number {
  ...
  close(): void {
    ...
    this.#failNegotiations('closed by caller')
    this.#subs.clear()
    this.#progress.clear()
  ...
  #down(reason: string, idleDeath?: { silence: number }): void {
    ...
    // A relay keeps no negentropy state across connections, and stored events
    // it had not finished sending will be sent again for the next REQ.
    this.#failNegotiations('connection lost')
    for (const progress of this.#progress.values()) progress.eoseAt = 0
  ...
  /** Whether anything is subscribed here, which keeps an unconfigured relay's socket open. */
  get hasSubscriptions(): boolean {
    return this.#subs.size > 0
  }

  /** How far a subscription has got on the current connection, or null if it is not held. */
  progressOf(id: string): SubscriptionProgress | null {
    if (!this.#subs.has(id)) return null
    return { ...(this.#progress.get(id) ?? { reqAt: 0, eoseAt: 0 }) }
  }

  // --- negentropy (NIP-77) --------------------------------------------------

  /**
   * Open a negentropy exchange. Only on an open socket: the owner decides
   * when to reconcile, and a reconnect would discard the exchange anyway.
   */
  negOpen(id: string, session: NegentropySession): void {
    if (this.#state !== 'open') {
      session.onError('not connected', false)
      return
    }
    this.#negotiations.set(id, { ...session, answered: false })
    this.#rawSend(`["NEG-OPEN",${JSON.stringify(id)},${JSON.stringify(session.filter)},"${session.initial}"]`)
  }

  negSend(id: string, messageHex: string): void {
    if (!this.#negotiations.has(id)) return
    this.#rawSend(`["NEG-MSG",${JSON.stringify(id)},"${messageHex}"]`)
  }

  negClose(id: string): void {
    // Exchanges exist only on an open connection: one that closes fails them first.
    if (!this.#negotiations.delete(id)) return
    this.#rawSend(`["NEG-CLOSE",${JSON.stringify(id)}]`)
  }

  #failNegotiations(reason: string): void {
    const sessions = [...this.#negotiations.values()]
    this.#negotiations.clear()
    for (const session of sessions) session.onError(reason, false)
  }
  ...
  #sendReq(id: string, sub: SocketSubscription): void {
    ...
    this.#progress.set(id, { reqAt: Date.now(), eoseAt: 0 })
  ...
  #onMessage(data: unknown): void {
    ...
    switch (frame[0]) {
      ...
      case 'EOSE': {
        ...
        if (progress && progress.eoseAt === 0) progress.eoseAt = Date.now()
      ...
      case 'CLOSED': {
        const id = frame[1]
        if (typeof id !== 'string') return
        const reason = typeof frame[2] === 'string' ? frame[2] : ''
        const negotiation = this.#negotiations.get(id)
        if (negotiation) {
          this.#negotiations.delete(id)
          negotiation.onError(reason || 'closed', !negotiation.answered)
          return
        }
      ...
      case 'NEG-MSG': {
        const [, id, message] = frame as [string, unknown, unknown]
        if (typeof id !== 'string') return
        const negotiation = this.#negotiations.get(id)
        ...
        if (typeof message !== 'string') {
          this.#negotiations.delete(id)
          negotiation.onError('malformed NEG-MSG', false)
          return
        }
        negotiation.onMessage(message)
        return
      }
      case 'NEG-ERR': {
        const [, id, reason] = frame as [string, unknown, unknown]
        if (typeof id !== 'string') return
        const negotiation = this.#negotiations.get(id)
        if (!negotiation) return
        this.#negotiations.delete(id)
        negotiation.onError(typeof reason === 'string' ? reason : 'error', false)
        return
      }
      case 'OK': {
      ...
      case 'NOTICE':
        if (typeof frame[1] === 'string') {
          log.debug(`${this.url} notice: ${frame[1].slice(0, 200)}`)
          // A relay that does not know NIP-77 usually says so this way — the
          // only answer a NEG-OPEN gets — rather than with a NEG-ERR.
          if (UNKNOWN_COMMAND.test(frame[1])) {
            for (const [id, negotiation] of [...this.#negotiations]) {
              if (negotiation.answered) continue
              this.#negotiations.delete(id)
              negotiation.onError(frame[1].slice(0, 160), true)
            }
          }
        }
        return
