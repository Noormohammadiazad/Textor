...
import type { IRelayPool, PublishOutcome, SubCloser } from './relayPool'
...
export class NostrTransport {
  ...
  subscribeInbox(
    ...
    log.info(`subscribing to inbox since ${Math.max(0, sinceSec() - SYNC_REWIND_SEC)}`)
    // A function, so that every reconnect resumes from the sync cursor as it
    // stands then — rewound for NIP-59's backdating — rather than from the
    // cursor as it stood when the app started.
   ...
   *
   * Resolves shortly after half the relays have finished rather than waiting on
   * the slowest: the live subscription is open on every one of them and replays
   * the same window, so a straggler's messages still arrive — only the
   * "checking for messages" state stops waiting for them.
   */
  async fetchInbox(myPubkey: string, sinceSec: number, relays?: string[]): Promise<NostrEvent[]> {
    return this.pool.query(
      { kinds: [KIND_GIFT_WRAP], '#p': [myPubkey], since: Math.max(0, sinceSec - SYNC_REWIND_SEC) },
      relays,
      { maxWait: 8000, graceMs: 1200 },
    )
  }
