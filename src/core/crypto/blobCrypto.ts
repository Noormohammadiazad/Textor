 ...
 *
 * The key is random per copy, and seals exactly one plaintext. A copy can be
 * sent more than once — a sticker is — which sends the same ciphertext again
 * and never encrypts anything new under an old key. The key travels to the
 * recipient inside the NIP-44-encrypted rumor, so the ciphertext at rest and
 * in flight is useless without the conversation the message belongs to.
 */
...
/**
 * One sealed copy of a payload.
 *
 * The id names the bytes, not the ciphertext: the same photo sent twice, or
 * by two people, has one id and two keys. Chunks are stored and requested by
 * the pair, never by the id alone, or the second copy overwrites the first and
 * the first message can no longer be opened (ADR-052).
 */
export interface BlobRef {
  /** SHA-256 of the plaintext, hex. */
  id: string
  /** Commitment to the key and salt that sealed this copy: 16 bytes, hex. */
  copy: string
}
 ...
 * without revealing either, so it can travel in a frame and sit in an index.
 */
export const blobCopy = (envelope: Pick<BlobEnvelope, 'key' | 'salt'>): string =>
  bytesToHex(sha256(concatBytes(COPY_LABEL, hexToBytes(envelope.key), hexToBytes(envelope.salt)))).slice(
    0,
    32,
  )

export const blobRef = (envelope: Pick<BlobEnvelope, 'id' | 'key' | 'salt'>): BlobRef => ({
  id: envelope.id,
  copy: blobCopy(envelope),
})

/** A copy as one string, for maps keyed by copy. */
export const blobRefKey = (ref: BlobRef): string => `${ref.id}:${ref.copy}`
...
/** Whether a chunk authenticates under this envelope's key, at this index. */
export function chunkOpens(
  envelope: Pick<BlobEnvelope, 'id' | 'key' | 'salt' | 'chunks'>,
  index: number,
  ciphertext: Uint8Array,
): boolean {
  try {
    openChunk(envelope, index, ciphertext)
    return true
  } catch {
    return false
  }
